...
const registerUser = async function (req, res) {
  try {
    ...
    if (inValidPayload.errors.length) {
      ...
      const isAlreadyExist = await pool.query(query, value);

      if (isAlreadyExist.rows.length) {
        ...
        const query =
          "INSERT INTO users (name, email, password, phone, address, isAdmin) VALUES ($1, $2, $3, $4, $5, $6)";
        const values = [name, email, hashedPassword, phone, address, isAdmin];
...
async function userLogin(req, res) {
  try {
    ...
    if (validationErors.length) {
      ...
      const query = "SELECT * FROM users WHERE email = $1";
      const value = [req.body.email];
      const alreadyExist = await pool.query(query, value);
